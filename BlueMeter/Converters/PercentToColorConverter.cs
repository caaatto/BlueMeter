...
public sealed class PercentToColorConverter : IValueConverter
{
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        return Color.FromArgb((byte)scaled, color.R, color.G, color.B);
    }
    ...
    private static Color GetBaseColor(object? parameter)
    {
        ...
        if (parameter is ISolidColorBrush brush)
        {
            return brush.Color;
        }

        if (parameter is string colorString && Color.TryParse(colorString, out var parsedColor))
        {
            return parsedColor;
        }
