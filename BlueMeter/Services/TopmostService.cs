...
/// <summary>
/// Default Avalonia implementation of <see cref="ITopmostService"/>.
/// </summary>
/// <remarks>
/// Implementation details:
/// - Uses <see cref="Window.Topmost"/> for immediate Avalonia behavior.
/// - Applies native Z-order using SetWindowPos with HWND_TOPMOST/HWND_NOTOPMOST to ensure reliability.
/// - If the HWND is not created yet, defers native call until <see cref="Window.Opened"/>.
/// </remarks>
public sealed class TopmostService : ITopmostService
{
    ...
    public void SetTopmost(Window window, bool enable)
    {
        ...
        if ((window.TryGetPlatformHandle()?.Handle ?? nint.Zero) == nint.Zero)
        {
