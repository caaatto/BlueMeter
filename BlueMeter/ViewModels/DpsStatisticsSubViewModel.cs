...
public partial class DpsStatisticsSubViewModel : BaseViewModel, IDisposable
{
    ...
    public DpsStatisticsSubViewModel(ILogger<DpsStatisticsViewModel> logger, StatisticType type,
    ...
    public void SortSlotsInPlace()
    {
        ...
        try
        {
            // Use Post instead of Invoke to avoid blocking the UI thread during sorting
    ...
    protected StatisticDataViewModel GetOrAddStatisticDataViewModel(DpsData dpsData)
    {
        ...
        if (!DataDictionary.TryGetValue(dpsData.UID, out var slot))
        {
            ...
            Dispatcher.UIThread.Invoke(() => { Data.Add(slot); });
        }
    ...
    public void UpdateHistoricalData(IReadOnlyDictionary<long, DpsData> dpsDataDict, IReadOnlyDictionary<long, PlayerInfo> playerInfoDict)
    {
        ...
        Dispatcher.UIThread.Invoke(() => Data.Clear());
        DataDictionary.Clear();
    ...
    // MEMORY LEAK FIX: Implement IDisposable to unsubscribe from CollectionChanged event.
