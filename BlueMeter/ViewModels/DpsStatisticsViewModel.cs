...
public partial class DpsStatisticsViewModel : BaseViewModel, IDisposable
{
        ...
        {
            {
                ...
                new DpsStatisticsSubViewModel(logger, StatisticType.Damage, storage, debugFunctions)
            },
            {
                StatisticType.TakenDamage,
                new DpsStatisticsSubViewModel(logger, StatisticType.TakenDamage, storage, debugFunctions)
            },
            {
                StatisticType.Healing,
                new DpsStatisticsSubViewModel(logger, StatisticType.Healing, storage, debugFunctions)
            },
            {
    ...
    private void ConfigManagerOnConfigurationUpdated(object? sender, AppConfig newConfig)
    {
        ...
        {
            Dispatcher.UIThread.Invoke(() => AppConfig = newConfig);
        }
    }
    ...
    private void OnLoaded()
    {
        ...
        _logger.LogDebug(LogEvents.VmLoaded, "DpsStatisticsViewModel loaded");
        LoadPlayerCache();
    ...
    private void DataStorage_DpsDataUpdated()
    {
        if (!Dispatcher.UIThread.CheckAccess())
        {
            Dispatcher.UIThread.Post(DataStorage_DpsDataUpdated);
            return;
        }
    ...
    {
        _logger.LogTrace(LogEvents.VmUpdateData, "Update data requested: {Count} entries", data.Count);
    ...
    {
        _windowManagement.MinimizeDpsStatisticsWindow();
    }
    ...
    {
        _logger.LogDebug(LogEvents.VmRefresh, "Manual refresh requested");
    ...
    {
        _windowManagement.ShowSettings();
    }
    ...
    private void OpenEncounterHistory()
    {
        var viewModel = new EncounterHistoryViewModel(
            _chartDataService,
            _loggerFactory.CreateLogger<EncounterHistoryViewModel>(),
            _serviceProvider,
            _messageDialog);
        ...
        _windowManagement.ShowEncounterHistory(viewModel);
    }
    ...
    private void OpenPlayerSkillBreakdown(StatisticDataViewModel? player)
    {
        ...
        _windowManagement.ShowChartsForFocusedPlayer(player.Player.Uid);
    }
    ...
    private async Task LoadHistoricalEncounterAsync(Core.Data.Database.EncounterData encounterData)
    {
        try
        {
            ...
            await Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                await _windowManagement.LoadHistoricalEncounterInChartsAsync(encounterData.EncounterId);
        ...
        catch (Exception ex)
        {
            ...
            await _messageDialog.ShowErrorAsync("Error", $"Failed to load encounter: {ex.Message}");
        }
    }
    ...
    private void UpdateBattleDuration()
    {
        if (!Dispatcher.UIThread.CheckAccess())
        {
            Dispatcher.UIThread.Post(UpdateBattleDuration);
            return;
        }
    ...
    partial void OnScopeTimeChanged(ScopeTime value)
    {
        ...
        {
            Dispatcher.UIThread.Post(PerformUiUpdate);
        }
